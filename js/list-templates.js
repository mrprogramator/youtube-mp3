...
function getPlaylistsTemplate(playlists, videoId, videoTitle, channelTitle, publishedAt, imgUrl) {
    var html = '';
            ...
            + "<table style=\"width:100%;border-collapse:collapse\">"
                + "<tr>"
                    + "<td style=\"border-bottom:0.7px solid gray\">"
                        ...
                        +"</div>"
                    +"</td>"
                    + "<td style=\"width:290px;border-bottom:0.7px solid gray\">"
                        + "<a id=\"download-audio-btn\" class=\"video-action-btn\" onclick=\"getMedia('/get-mp3','mp3','" + videoId + "','" + videoTitle + "','download-audio-btn')\">"
                        ...
                        + "</a> "
                        + "<a id=\"download-video-btn\" class=\"video-action-btn\" onclick=\"getMedia('/get-mp4','mp4','" + videoId + "','" + videoTitle + "','download-video-btn')\">"
    ...
    if(playlists && playlists.length > 0){
        ...
        if(!videoId){
            html += "<div style=\"text-align:center;margin-top:25%;font-size:1.6rem\">"
                + (deviceLanguage && deviceLanguage.indexOf('es')>=0? "No hay listas de reproducci&oacute;n": "No playlists found") 
            + "</div>";
        }
    }
